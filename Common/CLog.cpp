...
CLog::CLog(std::string folder, std::string username)
	: _username(username)
	, _folder(folder)
{
	Open();
...
void CLog::Log(const char* message, va_list arglist)
{
	try 
	{
		time_t rawnow = time(NULL);
		...
		// É porque o usuario passou o dia
...
void CLog::BASE_GetFirstKey(const char * source, char * dest)
{
