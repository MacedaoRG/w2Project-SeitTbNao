			...
			if (m_bActive == 1)
			{
				...
				if (g_dwStartPlayTime != 0 && g_dwPausedTime == 0)
				{
					if (g_nDumpPacketSize > g_nCurrentPos)
					{
						...
						{
							g_nCurrentPos += 4;
							PacketHeader* pStd = (PacketHeader*)&g_pDumpPacket[g_nCurrentPos];
...
HRESULT NewApp::MsgProc(HWND hWnd, DWORD uMsg, DWORD wParam, int lParam)
{
	...
	case WM_USER + 1:
	{
		...
		while (1)
		{
			...
			PacketHeader* pStd = (PacketHeader*)Msg;

			unsigned int dwServerTime = m_pTimerManager->GetServerTime();
			g_dwServerTime = pStd->TimeStamp;
			...
			m_pObjectManager->OnPacketEvent(pStd->PacketId, Msg);
		}
	}
	break;
	case WM_USER + 100:
	...
	case WM_CLOSE:
	{
		if (g_pCurrentScene != nullptr && g_pCurrentScene->m_eSceneType == ESCENE_TYPE::ESCENE_FIELD)
		{
			if (g_dwStartQuitGameTime == 0)
			{
				...
				stDelayStart.Header.ClientId = g_pCurrentScene->m_pMyHuman->m_dwID;
