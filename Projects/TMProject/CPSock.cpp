...
char* CPSock::ReadMessage(int* ErrorCode, int* ErrorType)
{
	*ErrorCode = 0;
	...
	if (Size >= RECV_BUFFER_SIZE || Size < sizeof(PacketHeader))
	{
		nRecvPosition = 0;
		nProcPosition = 0;
		*ErrorCode = 2;
	...
	for (int i = sizeof(int); i < Size; i++, pos++)
	{
		...
		int rst = pos % 256;
		unsigned char Trans = pKeyTable[rst * 2 + 1];
...
int CPSock::AddMessage(char* pMsg, int Size, int FixedKeyWord)
{
	...
	if (Size + nSendPosition < SEND_BUFFER_SIZE)
	{
		...
		auto packet = reinterpret_cast<PacketHeader*>(pMsg);
		packet->Size = Size;
		packet->Key = iKeyWord;
		packet->CheckSum = 0;
