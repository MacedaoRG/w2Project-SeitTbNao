...
{
    PacketHeader* pMsgStandard = (PacketHeader*)Msg;
    if (Msg != nullptr 
        && (LastSendTime + 1000 < CurrentTime || pMsgStandard->PacketId != MSG_Action_Opcode || g_usLastPacketType != MSG_Action_Opcode)
        && (LastSendTime + 1000 <= CurrentTime || pMsgStandard->PacketId != MSG_Attack_Multi_Opcode && pMsgStandard->PacketId != MSG_Attack_One_Opcode
        && pMsgStandard->PacketId != MSG_Attack_Two_Opcode || g_usLastPacketType != MSG_Attack_Multi_Opcode && g_usLastPacketType != MSG_Attack_One_Opcode && g_usLastPacketType != MSG_Attack_Two_Opcode))
    {
        g_pSocketManager->SendOneMessage(Msg, Size);
        g_usLastPacketType = pMsgStandard->PacketId;
    }
}
