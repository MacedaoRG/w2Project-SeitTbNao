...
int TMSelectServerScene::OnControlEvent(unsigned int idwControlID, unsigned int idwEvent)
{
	...
	switch (idwControlID)
	{
		case B_LOGIN_OK:
		{
			...
			sprintf_s(stAccountLogin.Login, "%s", pEditID->GetText());
			sprintf_s(stAccountLogin.Password, "%s", pEditPassword->GetText());
			sprintf_s(g_pObjectManager->m_szAccountName, "%s", stAccountLogin.Login);
			g_pObjectManager->m_szAccountPass[0] = stAccountLogin.Password[0];
			g_pObjectManager->m_szAccountPass[1] = stAccountLogin.Password[1];
			...
			g_pSocketManager->SendOneMessage(reinterpret_cast<char*>(&stAccountLogin), sizeof p20D);
...
int TMSelectServerScene::OnPacketEvent(unsigned int dwCode, char* buf)
{
	...
	auto packet = reinterpret_cast<PacketHeader*>(buf);
	...
	if (packet->PacketId == MSG_CNFAccountLogin_Opcode)
	{
		m_pMessagePanel->SetVisible(0, 1);
		g_pTimerManager->SetServerTime(packet->TimeStamp);
	...
	if (packet->PacketId == 0x11D || packet->PacketId == 0x11C)
	{
		if (packet->PacketId == 0x101)
			m_pLoginPanel->SetVisible(1);
