...
bool CUser::RequestAcceptParty(PacketHeader *Header)
{
	...
	CMob *liderSpw = &pMob[p->liderID];
	CMob *targetSpw = &pMob[clientId];

	if(liderSpw->Leader != 0)
	{
		SendClientMessage(clientId, "O usuario não é líder do grupo.");
	...
	if(i >= 12)
	{
		SendClientMessage(clientId, "Grupo esta cheio.");

		return true;
	}
	
	if(pMob[clientId].Target.X >= 2608 && pMob[clientId].Target.X <= 2647  && pMob[clientId].Target.Y >= 1708 && pMob[clientId].Target.Y <= 1748)
		return true;
