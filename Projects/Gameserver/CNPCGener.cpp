...
bool LoadMob(char *mobName, int mobIndex, bool mob)
{
	...
	{
		if(mob)
			fread(&mGener.pList[mobIndex].Leader, 1, sizeof STRUCT_MOB, hFile);
		else
			fread(&mGener.pList[mobIndex].Follower, 1, sizeof STRUCT_MOB, hFile);
...
{ 
	...
	if(!newFile)
		MessageBoxA(NULL, "Não sera possível gerar \"NPCGener_new.txt\"", "Fail ReadNPCGener", 4096);
    ...
    while(fgets(tmp, sizeof(tmp), pFile))
	{
		...
		if(index != -1 )
		{
			STRUCT_NPCGENERATOR *genMob = &pList[index];
