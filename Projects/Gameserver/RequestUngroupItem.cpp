...
bool CUser::RequestUngroupItem(PacketHeader *Header)
{
	...
	INT32 amount = GetItemAmount(&pMob[clientId].Mobs.Player.Inventory[p->SlotID]);
	...
	Log(clientId, LOG_INGAME, "Desagrupando o item %s. Tirando %d unidades. Novo item colocado no slot %d", pMob[clientId].Mobs.Player.Inventory[p->SlotID].toString().c_str(), p->Amount, Slot);
