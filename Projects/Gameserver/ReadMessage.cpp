    ...
    *ErrorCode = 0;
    ...
    if(Size > MAX_MESSAGE_SIZE || Size < sizeof(PacketHeader))
    {
        ...
        *ErrorType   =  Size;
		...
		if (User.Username[0])
			Log(clientId, LOG_INGAME, "Enviado pacote maior que o maximo ou menor que o Header. Size: %d", Size);
...
bool CUser::ReadMessage()
{
	...
	// Checa se esta realmente conectado
	...
	for (; len > 0; len -= lastSize)
	{
		...
		{
			Log(clientId, SERVER_SIDE, "HashKey invalida - %X : %X", hash, Header->Key);
