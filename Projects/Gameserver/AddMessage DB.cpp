...
bool AddMessageDB(BYTE *pBuffer, UINT32 packetSize)
{
	...
	if (data == nullptr)
	{
		Log(SERVER_SIDE, LOG_INGAME, "Buffer de destino invalido");
		return false;
	}
	...
	{
		Log(SERVER_SIDE, LOG_INGAME, "Buffer da DB esta full. Atual: %d. Tamanho do pacote %d", size, packetSize);
		return false;
	}

	if (!sData.Socket)
	{
		Log(SERVER_SIDE, LOG_INGAME, "Socket da DB esta invalido invalido", size, packetSize);
		return false;
	}

	if (packetSize < 12 || pHeader->PacketId == 0)
	{
		Log(SERVER_SIDE, LOG_INGAME, "Pacote invalido, packetSize: %d. PacketId %03X", packetSize, pHeader->PacketId);
		return false;
	}
