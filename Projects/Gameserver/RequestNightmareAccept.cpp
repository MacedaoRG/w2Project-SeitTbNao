...
bool CUser::RequestNightmareAccept(PacketHeader* header)
{
	...
	{
		Log(SERVER_SIDE, LOG_HACK, "Enviou índice invalido ao tentar entrar no Pesadelo novamente");
	...
	if (nightmareIndex == 0)
	{
		...
		if (pMob[clientId].Mobs.Nightmare[0].X != 0 && pMob[clientId].Mobs.Nightmare[0].Y != 0)
		{
			posX = pMob[clientId].Mobs.Nightmare[0].X;
			posY = pMob[clientId].Mobs.Nightmare[0].Y;
		}
	}
	else if (nightmareIndex == 1)
	{
		...
		if (pMob[clientId].Mobs.Nightmare[1].X != 0 && pMob[clientId].Mobs.Nightmare[1].Y != 0)
		{
			posX = pMob[clientId].Mobs.Nightmare[1].X;
			posY = pMob[clientId].Mobs.Nightmare[1].Y;
		}
	}
	else if (nightmareIndex == 2)
	{
		...
		if (pMob[clientId].Mobs.Nightmare[2].X != 0 && pMob[clientId].Mobs.Nightmare[2].Y != 0)
		{
			posX = pMob[clientId].Mobs.Nightmare[2].X;
			posY = pMob[clientId].Mobs.Nightmare[2].Y;
		}
	}
