...
BOOL CPSock::SendMessage(void)
{
	...
	{
		Log("Buffer invalido ao tentar enviar pacote. nSendPosition: %d. nSentPosition: %d", nSendPosition, nSentPosition);
		nSendPosition = 0;
		nSentPosition = 0;
	}
...
char* CPSock::ReadMessage(int *ErrorCode, int* ErrorType)
{
	*ErrorCode = 0;
	...
	if (Size > MAX_MESSAGE_SIZE || Size < sizeof(PacketHeader))
	{
		...
		*ErrorCode = 2;
		*ErrorType = Size;
		Log("Pacote invalido recebido. %hd > %d ou %hd < %u. Possível id do pacote: 0x%03X", Size, MAX_MESSAGE_SIZE, Size, sizeof PacketHeader, *((unsigned short*)(recvBuffer + nProcPosition + 4)));
		return NULL;
	}
